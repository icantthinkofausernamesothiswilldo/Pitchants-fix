...
public class EnchantmentPinDown extends Enchantment {
    ...
    public void onEntityDamaged(EntityLivingBase user, Entity target, int level) {
        ...
        {
            if((((EntityLivingBase) target).getLastDamageSource()).isProjectile() && !(user.isPotionActive(PotionInit.VENOM) || EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SOMBER, user.getItemStackFromSlot(EntityEquipmentSlot.LEGS)) > 0)) {
                if (handled) {
                    handled = false;
                    return;
                }
        ...
        {
            if (((MultiPartEntityPart) target).parent instanceof Entity)
                onEntityDamaged(user, (Entity) ((MultiPartEntityPart) target).parent, level);
        }
    }
}
